...
void DessineUneLigneBus(t_liste_station l, LTexture mySpriteArretBus, SDL_Renderer* gRenderer){
    t_liste_station maLigneBus = l;
    t_liste_station prochaineStation, prochainTroncon;
    int idframe = 0;
    int premier_passage = 1; // pour détecter un tour complet sur liste circulaire
    ...
    do {
        ...
        premier_passage = 0;
    } while (!ligneBusVide(maLigneBus) && (maLigneBus != l || premier_passage));
}
